...
| ---------- | ----------- | -------- | -------- | ----- |
| `tiktok_ads__ad_report`, `tiktok_ads__advertiser_report` | Bug Fix | `ad_name`, `advertiser_id`, `advertiser_name`, `campaign_id`, and `ad_group_id` were `null` for TikTok Smart+ ads | These fields now populate correctly for Smart+ ads | TikTok stores Smart+ ads in `creative_history`. Both models now include `creative_history`, using `creative_id` as the reporting `ad_id`, so Smart+ ads resolve correctly. |
| `tiktok_ads__url_report` | Bug Fix, New Column | `ad_name`, `base_url`, `url_host`, `url_path`, and `utm_*` were `null` for Smart+ ads | These fields now populate correctly for Smart+ ads, and a new `landing_page_urls` column is included | Smart+ ads are matched through `creative_history`, then joined to `smart_plus_ad_history` for landing page data. Because a Smart+ ad can have multiple landing pages, `landing_page_urls` preserves the full comma-separated set, while the existing single-URL fields continue to use one URL for backwards compatibility. |
| `stg_tiktok_ads__creative_history_tmp`, `stg_tiktok_ads__creative_history` | New Tmp/Staging Model | | | |
...
- Removed the dependency on the `fivetran/tiktok_ads_source` package.
  - All functionality from the source package has been merged into this transformation package for improved maintainability and clarity.
  - If you reference `fivetran/tiktok_ads_source` in your `packages.yml`, you must remove this dependency to avoid conflicts.
  - Any source overrides referencing the `fivetran/tiktok_ads_source` package will also need to be removed or updated to reference this package.
  - Update any tiktok_ads_source-scoped variables to be scoped to only under this package. See the [README](https://github.com/fivetran/dbt_tiktok_ads/blob/main/README.md#change-the-build-schema) for how to configure the build schema of staging models.
...
Migrated `freshness` from a top-level source property to a source `config` in alignment with [recent updates](https://github.com/dbt-labs/dbt-core/issues/11506) from dbt Core ([Source PR #31](https://github.com/fivetran/dbt_tiktok_ads_source/pull/31)). This will resolve the following deprecation warning that users running dbt >= 1.9.6 may have received:
...
If you are using dbt Core < 1.9.6 and want to continue running TikTok Ads freshness tests, please elect **one** of the following options:
  ...
  2. Do not upgrade your installed version of the `tiktok_ads` package. Pin your dependency on v0.8.1 in your `packages.yml` file.
